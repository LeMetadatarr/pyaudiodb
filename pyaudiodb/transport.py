...
- **Opt-in disk caching** — GET/HEAD responses with status ``200`` are stored
  on disk when ``PYAUDIODB_HTTP_CACHE`` is set, using stdlib only (hashlib,
  json, base64).
...
``PYAUDIODB_HTTP_CACHE``
    ...
    ``~/.cache/pyaudiodb/http`` is used.

``PYAUDIODB_HTTP_CACHE_TTL``
...
LOG = logging.getLogger("pyaudiodb.transport")

_DEFAULT_CACHE_DIR = Path.home() / ".cache" / "pyaudiodb" / "http"
_DEFAULT_TTL = 86400
...
def _cache_from_env() -> Optional[DiskCache]:
    ...
    env = os.environ.get("PYAUDIODB_HTTP_CACHE", "").strip()
    if not env:
        return None
    ttl_raw = os.environ.get("PYAUDIODB_HTTP_CACHE_TTL", str(_DEFAULT_TTL)).strip()
    try:
        ttl_val = int(ttl_raw)
        ttl: Optional[int] = ttl_val if ttl_val > 0 else None
    except ValueError:
        LOG.warning("Invalid PYAUDIODB_HTTP_CACHE_TTL %r — using default", ttl_raw)
        ttl = _DEFAULT_TTL
...
def clear() -> int:
    ...
    d = _resolve_dir(os.environ.get("PYAUDIODB_HTTP_CACHE", ""))
    if not d.is_dir():
        return 0
    count = 0
